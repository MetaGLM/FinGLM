from app.build_prompt import build_nl2sql_prompt_type1, build_chatgpt_nl2sql_prompt, build_nl2sql_prompt_type2
from app.config import DB_PATH
from app.vector_search import vector_search
from app.keywords import comp_short_dict,comps,comps_short
from app.formulas import formula_dict
from app.utils import json_lines_dump,edit_distance,is_zh, is_int, is_float, find_dot, algin_float_string, year_add, lcs_sub
# from app.load_model import ask_nl2sql_chatglm2
from db.db_schema import schema, schema_base, schema_fin, schema_emp, schema_meta, schema_edu
...
stopwords = set([i.strip() for i in open("resources/stopwords.txt", encoding="utf-8").readlines() if i])
stopwords_v2 = set([i.strip() for i in open("resources/stopwords_v2.txt", encoding="utf-8").readlines() if i])
...
def insert_hint_to_query(query, word, hint):
    word_idx = query.find(word)
    word_tail_idx = word_idx + len(word)
    return query[:word_tail_idx] + f"({hint})" + query[word_tail_idx:]

def query_db(query, query_parse_result, temperature=0.1, **kwargs):
    cols = prepare_columns_for_sql(query, query_analyze_result)
    formulas = prepare_formula_for_sql(query, query_analyze_result)
    query = prepare_query_for_sql(query, query_analyze_result)

    prompt = build_nl2sql_prompt(TABLE_NAME, query, cols, formulas)
    sql = ask_nl2sql_chatglm2(prompt, temperature=temperature, **kwargs)
...
def prepare_prompt(query, query_type, query_analyze_result, use_chatgpt=False):
    ...
    # if use_chatgpt:
    #     return build_chatgpt_nl2sql_prompt(query, cols)
    # else:
    #     return build_nl2sql_prompt(query, cols)



def prepare_type2_prompt(query, query_analyze_result):
    if query_analyze_result == None:
        query_analyze_result = get_query_analyze_result(query)
    cols = prepare_columns_for_sql_v2(query, query_analyze_result)
    formulas = [i[0] for i in vector_search(list(formula_dict.keys()), query, "formula", k=1)]
    if formulas:
        formula = formula_dict.get(formulas[0], {}).get("raw_formula", "")
    else:
        formula = ""
    prompt = build_nl2sql_prompt_type2(query, cols, formula)
    return prompt
