...
def load_model_from_modelscope():
    ...
    llm = Model.from_pretrained(LLM_NAME, device_map='auto', cache_dir=LLM_CACHE_DIR, revision=LLM_REVISION)
    pipe = pipeline(task=Tasks.chat, model=llm)
    return pipe

def ask_chatglm2_from_modelscope(prompt, **kwargs):
    inputs = {'text':prompt, 'history': []}
    return pipe(inputs, **kwargs)["response"]
...
global llm, pipe, tokenizer

ask_chatglm2 = ask_chatglm2_from_modelscope
if ASK_SWITCH == "transformers":
    ...
    pipe = load_model_from_modelscope()
