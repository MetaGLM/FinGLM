...
def get_response(company_infos):
    ...
    open_querier = open_query(common_querier.model, tokenizer, excels)
    open_answers = open_querier.run_query(questions, company_infos, batch_size=1)
    result = []
    for idx in range(len(company_infos)):
        if company_infos[idx]['类型'] == '财务问题' and finance_answers[idx] is not None:
            result.append(finance_answers[idx])
        elif company_infos[idx]['类型'] == '公司问题' and company_answers[idx] is not None:
            result.append(company_answers[idx])
        elif company_infos[idx]['类型'] == '人员问题' and personal_answers[idx] is not None:
            result.append(personal_answers[idx])
        elif company_infos[idx]['类型'] == '常识问题' and common_answers[idx] is not None:
            result.append(common_answers[idx])
        elif company_infos[idx]['类型'] == '开放问题' and open_answers[idx] is not None:
            result.append(open_answers[idx])
        elif company_infos[idx]['类型'] == '查询问题' and complex_answers[idx] is not None:
            result.append(complex_answers[idx])
        else:
            # print(idx, questions[idx] ,company_infos[idx])
            result.append({"id":idx, "question" : questions[idx], "answer": "不知道"})
    return result
...
if __name__ == '__main__':
    ...
    # company_infos = set_stock_code(load_extracted())
    # print(company_infos)
    # with open('company_infos.jsonl', 'w', encoding = 'utf-8') as f:
    #     json.dump(company_infos, f, ensure_ascii=False, indent=4)
    # load_frames()