...
def preprocess_dataset(
    ...
    def format_example(examples): # support question with a single answer or multiple answers
        for i in range(len(examples["prompt"])):
            prompt, query, answer = examples["prompt"][i], examples["query"][i], examples["response"][i]
            query = examples["prompt"][i] + examples["query"][i] if examples["query"][i] else query
            if examples['category'][i] == 0:
                query = template_manager.get_template(examples['category'][i]).format(question=examples['query'][i])
            else:
                query = template_manager.get_template(examples['category'][i]).format(prompt=examples['prompt'][i], question=examples['query'][i])
            prompt = ""
            prompt += "{}".format(query)
